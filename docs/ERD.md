...
- 최종 수정일: 2026-08-21
- 포함 도메인: 회원·인증, 숙소, 객실, 예약, 결제, LLM 여행 계획
- 제외: 검색, 리뷰, 찜, 쿠폰, 포인트, 채팅, 알림, AI 리뷰 요약, 별도 환불 이력
...
- 숙소 논리 삭제 시 이미지 행과 S3 객체는 유지된다. 물리 삭제를 수행할 경우에만 FK Cascade로 이미지 행이 함께 삭제된다.
...
- 객실 논리 삭제 시 이미지 행과 S3 객체는 유지된다. 물리 삭제를 수행할 경우에만 FK Cascade로 이미지 행이 함께 삭제된다.

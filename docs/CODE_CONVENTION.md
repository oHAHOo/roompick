...
2. 현재 MVP에서는 숙소와 객실 더미데이터를 각각 1개만 사용하고 검색 기능을 구현하지 않는다.
...
- 검색, 리뷰, AI 등 MVP 이후 기능은 구현 시점에 도메인 패키지를 추가한다.
...
- 목록 조회에는 페이징을 우선 고려한다. 단, 현재 MVP에는 검색·목록 기능을 임의로 추가하지 않는다.
- N+1 문제를 확인하고 필요한 경우에만 fetch join이나 EntityGraph를 적용한다.
- 모든 관계를 무조건 EAGER로 설정하지 않는다.
- 복잡한 조회가 생기면 QueryDSL을 검토하되 MVP에서는 필요하지 않은 검색 쿼리를 만들지 않는다.

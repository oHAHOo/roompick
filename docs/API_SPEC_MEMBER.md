...
이 문서는 RoomPick MVP의 회원가입·로그인 API와 JWT 인증·인가 구조를 정의한다. 관리자 숙소·객실 등록 API는 minjae123123 담당의 `docs/API_SPEC_ADMIN.md`, 숙소·객실·예약 조회·예약 API는 임선구 담당의 `docs/API_SPEC_OWNER.md`에서 관리한다.
...
3. 로그아웃 API는 MVP 범위에서 제외하며, 추후 JWT + Redis(블랙리스트) 방식으로 구현할 예정이다.
...
- [ ] Refresh Token으로 Access Token을 재발급하는 API를 MVP에 포함할지
- [ ] 로그아웃(JWT + Redis 블랙리스트)을 어느 시점에 구현할지

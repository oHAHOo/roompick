...
관리자 등록·논리 삭제 API는 `docs/API_SPEC_ADMIN.md`에서 관리한다. 장소 후보·위치 기반 숙소 검색 외의 관리자용 숙소·객실 수정·관리 목록 API는 포함하지 않는다.
...
- 인증 없이도 호출할 수 있지만, 요청에 `ADMIN` 토큰이 실려 있으면 숙소 운영 상태와 무관하게 조회할 수 있고 `INACTIVE` 객실도 목록에 포함되며 각 항목에 `status`가 포함된다.
...
- 인증 없이도 호출할 수 있지만, 요청에 `ADMIN` 토큰이 실려 있으면 `INACTIVE` 숙소도 조회할 수 있고, 응답에 `status`가 포함된다. `ADMIN`이 아니면 지금까지처럼 `409 ACCOMMODATION_INACTIVE`/`404`다.
...
- 객실 운영 상태는 일반 사용자 응답에는 포함하지 않는다.
- 인증 없이도 호출할 수 있지만, 요청에 `ADMIN` 토큰이 실려 있으면 `INACTIVE` 객실도 조회할 수 있고, 응답에 `status`가 포함된다. `ADMIN`이 아니면 지금까지처럼 `404 ROOM_NOT_FOUND`다.
- `imageUrls`는 등록 순서대로 정렬된 전체 이미지 URL 목록이며, 등록된 이미지가 없으면 빈 배열이다.
